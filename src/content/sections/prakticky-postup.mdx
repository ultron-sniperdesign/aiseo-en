...
1. **Technical readiness** — indexovatelnost, povolení AI crawlerů (`GPTBot`, `PerplexityBot`, `ClaudeBot`, `Google-Extended`) v `robots.txt`, technické zdraví (Core Web Vitals, mobile, HTTPS). Bez tohoto základu se AI systémy k webu nedostanou.
2. **Content extraction** — krátké odpovědi 40–60 slov hned po nadpisech, jasné definice ve formátu „X je…", pyramidové uspořádání obsahu (nejdůležitější věci nahoru). Tato vrstva má **nejvyšší dopad na citation rate** v AI nástrojích.
3. **Entity clarity** — konzistentní brand zápis napříč webem, schema.org `Organization` a `Person` markup, propojené identifikátory (Wikidata, oborové katalogy). Pomáhá AI systémům jednoznačně identifikovat, kdo jste.
4. **AI trust signals** — fact-density (čísla, procenta, data), citace zdrojů, autorské profily u YMYL témat, datace tvrzení, brand mentions v autoritních komunitách (Reddit, Quora, oborové weby).
5. **Citation optimization** — měření AI citation share, A/B testy answer blocků, iterace podle dat z GSC a manuálního auditu ChatGPT/Perplexity.
