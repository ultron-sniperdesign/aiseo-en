...
| **TR-25** | `a text`, `SEO texts` (= článek/copy) | `a piece`, `an article`, `SEO copy`, `content` | kalk („text") | České „text" pro článek/copy → AJ `piece/article/copy/content`. „a text" v US AJ = zpráva nebo literární dílo, ne marketingový článek. |
| **TR-26** | `you live on [traffic]` | `you depend on / rely on [traffic]` | kalk („žít z") | „žít z provozu" → `depend on / rely on traffic`, ne `live on`. |
...
| **`/generative-engine-optimization/` (geo.mdx)** | 2026-05-24 | gpt-5.5 | 8 (P1) + 10 (P2) | P1: 8 auditor + proaktivně TR-1 (3× e-shop), TR-3, TR-21, TR-23, TR-24 · P2: 10 | Potvrzeno TR-16 (`signal … showing where`→`signal that shows`), TR-17 (`concrete implementation area`→`specific`). **Nový mini-vzor:** `citations on [queries]`→`for [queries]` (2×). One-offs: `post-milestone project`→`project for later`, `sites that hallucinate themselves`→`…publish hallucinations`, `hit good SEO`→`get SEO right`, `brand-search popularity`→`branded search demand`, `artificially invented questions`→`made-up questions`, `from the first version`→`from the start`. Verdikt P2: „mostly natural US English, punchy native style". |
| **`/answer-engine-optimization/` (aeo.mdx)** | 2026-05-24 | gpt-5.5 | 10 (P1) + 11 (P2) | P1: 10 auditor + proaktivně TR-1 (4× e-shop), TR-21, TR-23, TR-24, TR-17 · P2: 11 | **Nový vzor:** `live on [traffic]`→`depend on / rely on [traffic]` (kalk „žít z"). One-offs: `AI Overviews appears`→`the AI Overviews panel appears` (číslo/shoda), `be in the citation`→`be cited`, `priorities go to`→`prioritize`, `inherit it 1:1`→`carry over directly`, `in the first wave of the SERP`→`near the top of the SERP`, `systematic presence`→`consistent visibility`, `activation is less predictable`→`are less predictable`, `where … raises vs lowers CTR`→`when …`, `for the advanced stage`→`once you're more advanced`, `article #1`→`the first article`. Verdikt P2: „mostly natural US-English". |
| **`/ai-seo/` (aio.mdx)** | 2026-05-24 | gpt-5.5 | 12 (P1, 1 zamítnut) + 7 (P2) | P1: 11 auditor + proaktivně TR-1 (5× e-shop), TR-21, TR-7, `live on`→`depend on`, `concrete impl.`→specific · P2: 7 | **TR-25 (`text`→`piece/copy`) + TR-26 (`live on`→`depend on`) odtud. Zamítnut F01** `umbrella over`→`umbrella for` (0.78 conf) — „umbrella over" je zavedený termín napříč webem, měnit by rozbilo konzistenci. One-offs: `goes in with no extra cost`→`folded in`, `framework that answers '…'`→`framework for answering: "…?"`, `into which Gemini pulls`→split na 2 věty, `conversely lose clicks … takes the answer`→`serves the answer directly`, `Featured Snippets = position 0`→`(position 0)`, `strategy goes through layers`→`covers`. Verdikt P2: „mostly natural US English". |
| **`/ai-seo-playbook/` (prakticky-postup.mdx)** | 2026-05-24 | gpt-5.5 | 9 (P1) + 5 (P2) | P1: 9 auditor + 1× e-shop · P2: 5 | **Vzor: český slovosled příslovce** (`identify unambiguously`→`unambiguously identify`, `understand faster and more accurately`→`understand more quickly and accurately`, `has a typically higher chance`→`typically has a higher chance`). One-offs: `spinning in dead ends`→`spinning your wheels`, `write content GEO-friendly right away`→`write GEO-friendly content from the start`, `manual sheet`→`spreadsheet`, `base content`→`foundational content`, `in the citation`→`cited`, `AI Overviews embrace or bypass`→`embrace or bypass AI Overviews`. Verdikt P2: „mostly natural US English". |
| **`/decision-matrix/` (rozhodovaci-matice.mdx)** | 2026-05-24 | gpt-5.5 | 6 (P1) + 5 (P2) | P1: 6 auditor + proaktivně ~12× e-shop, 4× `Detail in` (TR-23), `Concretely`→`Specifically` (TR-17), `opening hours`→`business hours` (TR-22) · P2: 5 | One-offs: `conversion-valuable`→`valuable for conversions`, `has no business in [an investment]`→`has no business making`, `index authority`→`search authority`, `make do with the extract`→`get enough from the excerpt`, `honest FAQ`→`useful FAQ`, `base content`→`foundational content`, `Progress by X`→`based on X`, `embrace` jako postponed noun→sloveso. Verdikt P2: „mostly natural, native-sounding US English". **Tím jsou všechny first-wave stránky (homepage + pillar + 6 sekcí) hotové: 2 průchody, 0 e-shop, 0 reziduálních kalků.** |
| **blog (pilot): `/blog/how-to-turn-off-ai-overviews/`** (z `jak-vypnout-ai-overview`) | 2026-05-26 | gpt-5.5 | 5 (P1) + 5 (P2) | **full CZ→EN překlad** + 2-pass | **Pilot blog vlny.** De-CZ (pryč „v Česku"/CZ Trends framing/CZ audit sales-pitch), AI Overviews terminologie (ne „AIO"), interní odkazy → EN slugy (`/ai-seo/`, `/decision-matrix/`, `/ai-seo-playbook/`), nový EN slug. Překlad vyšel čistě (10 jemných nálezů). One-offs: `side damage`→`unintended side effects`, `how usable your content is`→`how your content can be used`, `part of [queries]`→`a subset of`, `AI Overviews answers`→`an AI Overview answers`, `indicative signal`→`indication`. **Renderování čeká na A (blog chrome §5); obsah zabankován.** |
